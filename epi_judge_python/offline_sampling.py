...
def random_sampling(size: int, elements: List[int]) -> None:
    for i in range(size):
        randomIdx = random.randint(i, len(elements) - 1)
        elements[i], elements[randomIdx] = elements[randomIdx], elements[i]
